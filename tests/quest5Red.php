...
$mysqli = $db->getConnection();
	...
	if (isset($_POST["sbm"])) {
		...
		$sql = sprintf("UPDATE mag_test_short_answ SET answer='%s' WHERE id_quest='$qId'",$_POST["answer"]);
		//print("<br>$sql<br>");
				    ...
				    <div id="quest1" class="collapsed">
				        <form method='post' action='<?=$_SERVER['REQUEST_URI']?>' enctype="multipart/form-data" >
				            <?php
				                ...
				                print("<br>");
				                $sql = "SELECT * FROM mag_test_quest WHERE id_q=$qId";
				                $res = $mysqli->query($sql);
				                $row = $res->fetch_assoc();
				                $sql_answer = "SELECT * FROM mag_test_short_answ WHERE id_quest=$qId";
